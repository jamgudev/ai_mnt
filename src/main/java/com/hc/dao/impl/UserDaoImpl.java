...
//		});
        ...
        return this.getHibernateTemplate().execute(new HibernateCallback<List<Worker>>() {
            ...
            public List<Worker> doInHibernate(Session session) throws HibernateException {
                ...
                if (!place.equals("") && !place.equals(0)) {
                    ...
                    else if (place instanceof String)
                        hql += " where place.p_name = '" + place + "'";
                }
                return session.createQuery(hql).list();
            }
        });
    ...
    public List<MonitorData> getMostNumberByPlaceId(final Integer placeId) {
        return this.getHibernateTemplate().execute(new HibernateCallback<List<MonitorData>>() {
            @Override
            public List<MonitorData> doInHibernate(Session session) throws HibernateException {
                Query query = session.createQuery("from MonitorData where place.p_id = " + placeId + " and dt_from_time is not null" + " order by dt_id desc");
    ...
    public List<MonitorData> searchForToday(final String todayTime) {
        return this.getHibernateTemplate().execute(new HibernateCallback<List<MonitorData>>() {
            @Override
            public List<MonitorData> doInHibernate(Session session) throws HibernateException {
                List<MonitorData> list = session.createQuery("from MonitorData where dt_from_time like '" + todayTime + "%'" + " order by dt_id desc").list();
                return list;
            }
        });
    }
    ...
    @Override
    public MonitorData searchDetail(final Integer placeId, final Integer thredNum, final String level, final Integer maxNum, final String from, final String to) {
        return this.getHibernateTemplate().execute(new HibernateCallback<MonitorData>() {
            @Override
            public MonitorData doInHibernate(Session session) throws HibernateException {
                String sql = "select dt_changing_pn pn, dt_vd_url url from t_monitor_data where "
                        + "dt_place_id = " + placeId + " and ";
                if (maxNum != null && maxNum != 0)
                    sql += "dt_ppnb = " + maxNum + " and ";

                if (!StringUtils.isEmpty(level))
                    sql += "dt_alert_level = '" + level + "'" + " and ";

                if (thredNum != null && thredNum != 0)
                    sql += "dt_preset_pn = " + thredNum + " and ";

                sql += "dt_from_time = '" + from + "'" + " and dt_to_time = '" + to + "'" + " and ";
                ...
                if (o != null) {
                    String array = String.valueOf(o[0]);
                    String url = String.valueOf(o[1]);
                    MonitorData md = new MonitorData();
                    md.setDt_vd_url(url);
                    md.setDt_changing_pn(array);
                    return md;
                }
                return null;
            }
